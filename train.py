# === SAM-CLIP Enhanced Training Script with Dynamic FC Layer Creation and Detailed Comments ===
...
# === Precompute fixed text embedding for class label ===
text_tokens = clip.tokenize(["Powdery mildew"]).to("cuda")
with torch.no_grad():
...
def train_model(trainloader, valloader, dir_checkpoint, epochs):
    ...
    # === Dynamically create FC layers based on actual image encoder output shape ===
    with torch.no_grad():
        dummy = torch.zeros((1, 3, 1024, 1024)).cuda()
        dummy_emb = sam.image_encoder(dummy)
        C, H, W = dummy_emb.shape[1:]  # Extract channel and spatial dimensions
        fc_sam_to_clip = nn.Linear(C * H * W, 512).cuda()
        fc_fuse_to_decoder = nn.Linear(512 * 512, C * H * W).cuda()
    ...
    for epoch in pbar:
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            sam_img_emb = sam.image_encoder(imgs)
            B = sam_img_emb.shape[0]
            sam_feat = fc_sam_to_clip(sam_img_emb.view(B, -1))  # Flatten and project to CLIP dimension
            ...
            with torch.no_grad():
                clip_feat = clip_model.encode_image(clip_imgs)
                clip_feat = clip_feat / clip_feat.norm(dim=-1, keepdim=True)

            # === Feature fusion and projection back to SAM mask decoder space ===
            merged_feat = sam_feat + clip_feat
            text_feat_exp = clip_text_feat.expand(B, -1)
            dot = torch.bmm(merged_feat.unsqueeze(2), text_feat_exp.unsqueeze(1))  # B x 512 x 512
            fused = fc_fuse_to_decoder(dot.view(B, -1))
            img_emb = fused.view(B, C, H, W)
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                for i, data in enumerate(tqdm(valloader)):
                    ...
                    sam_img_emb = sam.image_encoder(imgs)
                    B = sam_img_emb.shape[0]
                    sam_feat = fc_sam_to_clip(sam_img_emb.view(B, -1))
                    ...
                    eval_loss += loss.item()
                    dsc_batch = dice_coeff_multi_class(pred.argmax(1).cpu(), torch.squeeze(msks.long(), 1).cpu(), args.num_cls)
                    dsc += dsc_batch
...
if __name__ == "__main__":
    ...
    train_dataset = Public_dataset(args, args.img_folder, args.mask_folder, train_img_list,
                                   phase='train', targets=[args.targets], normalize_type='sam', if_prompt=False)
    val_dataset = Public_dataset(args, args.img_folder, args.mask_folder, val_img_list,
