...
def load_model(checkpoint_dir: Path, device: str):
    ...
    # A checkpoint trained with model-parallel placement has the same weights;
    # inference places the complete model on the requested device.
...
def main():
    ...
    for img_path in tqdm(image_paths, desc="[INFO] Running inference", ncols=100):
        fname = img_path.stem
        pil_mask = evaluate_one(str(img_path), model, preprocess, device=device)
