    ...
    for epoch in pbar:
        ...
        if epoch%2==0:
            ...
            with torch.no_grad():
                ...
                elif (epoch-last_update_epoch)>3000:
                    # the network haven't been updated for 3000 epochs
...
if __name__ == "__main__":
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is required for training. Start the container with NVIDIA GPU access "
            "and run `sam-clip gpu-check` first."
        )
    if args.if_split_encoder_gpus:
        raise ValueError(
            "The legacy split-encoder mode is not supported by this single-GPU entrypoint. "
            "Use `train-ddp` for reproducible multi-GPU training."
        )
